let currentGravity = 0.5; // Store current gravity value
...
function createDirectionalPoint(c, ctx, x, y, color, dx, dy) {
    const ARROW_SCALE = 5;
    const PARTICLE_SIZE = 20; // wanted big visible particles
    
    ctx.beginPath();
    ctx.arc(x, y, PARTICLE_SIZE, 0, 2 * Math.PI);
    ctx.fillStyle = color;
    ctx.fill();
    // Adding a pretty white glow effect
...
function movePointsWithTrace(c, ctx, points, initialKeepTrace, initialGravity = 0.5) {
    ...
    function step() {
        ...
        points.forEach(point => {
            ...
            // Updated collision boundaries for larger particles
...
function updateGravity(value) {
    currentGravity = value; // Store the current gravity value
...
function updateTrace() {
    ...
    // Instead of recreating animation, just update the trace setting
